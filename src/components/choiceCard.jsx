...
const ChoiceCard = ({ data, index }) => {
  return (
    ...
    >
      {index <= 1 ? (
        <div className="choice-promo flex w-[10rem] bg-[#FF7724] items-center p-2 rounded-r-xl gap-2">
          {index === 0 ? (
      ...
      <div className="choice-index rounded-full border border-2 border-[#E1E4E600]">
      ...
      <div className="flex flex-col lg:flex-row justify-evenly mx-5">
        <div className="w-full lg:w-[15rem] mt-[3rem] flex flex-col justify-center items-center">
          <Image
            ...
            alt="logo"
          />
          <p className="text-sm text-[#626E79] text-center mt-2 font-normal">
            {data?.imageName}
          </p>
        </div>
        <div className="text-base w-full lg:w-[36rem] my-[1.5rem] mx-0 lg:mx-[2.5rem]">
          <p className="font-bold text-[#4B5665]">
            {data?.Heading}
            <span className="font-normal">{data?.headingContent}</span>
          </p>
          {data?.discount && (
            <div className="bg-[#F2F4F7] w-[4rem] rounded-md mt-4">
          ...
          {data.whyReason && <WhyWeLove whyReasons={data.whyReason} />}
